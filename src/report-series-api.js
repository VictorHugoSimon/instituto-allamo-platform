...
if(path.match(/^report-series\/[^/]+\/snapshot$/)&&request.method==='POST'){
  ...
  if(s.project_id){
    const details=(await DB.prepare('SELECT phase_key,milestone_key,phase_title,milestone_title,description,subdescription,phase_rank,milestone_rank,updated_at FROM project_milestone_details WHERE company_id=? AND project_id=? ORDER BY phase_rank,milestone_rank').bind(s.company_id,s.project_id).all()).results||[];
    const assets=(await DB.prepare('SELECT id,phase_key,milestone_key,phase_title,milestone_title,asset_type,title,description,link_url,file_name,mime_type,size_bytes,client_visible,created_at FROM project_milestone_assets WHERE company_id=? AND project_id=? AND archived_at IS NULL ORDER BY created_at').bind(s.company_id,s.project_id).all()).results||[];
    data._milestone_evidence={company_id:String(s.company_id),project_id:Number(s.project_id),details,assets};
  }
