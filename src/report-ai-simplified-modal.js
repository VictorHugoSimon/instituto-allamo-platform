...
`;
  ...
  function simplify(){
    ...
    const meeting=box.querySelector('#meeting');
    const sname=box.querySelector('#sname');
    const sdate=box.querySelector('#sdate');
    const stext=box.querySelector('#stext');
    const files=box.querySelector('#files');
    const inst=box.querySelector('#inst');
    const gen=box.querySelector('#gen');
    const msg=box.querySelector('#aimsg');
    if(!meeting||!gen)return;

    const meetingLabel=labelFor(meeting);
    if(meetingLabel){
      meetingLabel.querySelector(':scope')?.childNodes?.forEach?.(()=>{});
      ...
      const wrap=document.createElement('div');wrap.className='allamo-ai-primary';
      meetingLabel.parentNode.insertBefore(wrap,meetingLabel);wrap.appendChild(meetingLabel);
      const intro=document.createElement('div');intro.innerHTML='<h3>O que aconteceu na reunião?</h3><p>Cole o resumo, ata ou transcrição. Só isso já é suficiente para a IA preparar o rascunho do Report.</p>';
      wrap.insertBefore(intro,meetingLabel);
    }
    ...
    for(const el of [sname,sdate,stext,files,inst]){
      const l=labelFor(el);if(l&&l.parentNode!==body)body.appendChild(l);
    }
    ...
    const errors=[...box.querySelectorAll('.arai-error')];
    for(const err of errors){
      if(/modo gratuito|cloudflare|pdf\/imagem/i.test(err.textContent||'')){
        err.className='allamo-ai-status';
        err.innerHTML='<strong>Modo gratuito ativo.</strong> Para o uso diário, cole a reunião acima. Arquivos podem ser adicionados como apoio quando houver texto disponível.';
      }
    }

    gen.textContent='✨ Gerar rascunho do Report';
    gen.classList.add('allamo-ai-submit');
    const toolbar=gen.closest('.arai-toolbar');
    if(toolbar)toolbar.style.display='block';
    if(msg&&msg.previousElementSibling!==advanced){ /* mantém área de mensagens abaixo dos opcionais */ }

    meeting.placeholder='Cole aqui o resumo, ata ou transcrição da reunião. Ex.: Cadastros em andamento; TOTVS aguarda validação da Dual; Go-live sem alteração...';
    meeting.focus();
    box.dataset.simplified='1';
  }
