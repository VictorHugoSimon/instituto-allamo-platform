(()=>{
  ...
  const api=async p=>{const r=await fetch('/api/'+p,{headers:{authorization:'Bearer '+token()},cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);return r.json()};
  ...
  async function companyKpi(){
    if(document.querySelector('[data-allamo-company-kpi]'))return;
    ...
    const n=leaves.find(e=>/^\s*\d+\s*$/.test(e.textContent)); if(n)n.textContent='—';
    ...
    try{const companies=await api('companies');if(n)n.textContent=String(Array.isArray(companies)?companies.length:0)}catch(e){if(n)n.textContent='0'}
  }
