...
if(path==='public-report'&&request.method==='GET'){
  ...
  const co=await DB.prepare('SELECT * FROM companies WHERE id=? OR lower(CAST(id AS TEXT))=lower(?) LIMIT 1').bind(cid,cid).first();
  ...
  return json({data,gmuds,updates,company:{id:canonicalId,name:co.name},meta:{requested_company:cid,ref:row?.ref||null,updated_at:row?.updated_at||null,context_locked:true}});
}
