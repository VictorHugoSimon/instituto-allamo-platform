...
if (isAllamoStage) {
  // Assim como o bootstrap central, o schema de Reports é executado no máximo uma vez
  // por isolate. Isso evita várias sequências de DDL concorrentes quando a Central de
  // Reports carrega empresas, projetos, reports e work-items em paralelo.
  if (!globalThis.__allamoStageReportSchemaPromise) {
    globalThis.__allamoStageReportSchemaPromise = (async () => {
      ...
      // Campos dinâmicos multitenant. Campos de tela não exigem ALTER TABLE.
      await stageSafe("CREATE TABLE IF NOT EXISTS tenant_field_definitions (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, project_id INTEGER, entity_type TEXT NOT NULL, field_key TEXT NOT NULL, label TEXT NOT NULL, field_type TEXT NOT NULL DEFAULT 'text', config_json TEXT NOT NULL DEFAULT '{}', required INTEGER NOT NULL DEFAULT 0, client_visible INTEGER NOT NULL DEFAULT 1, rank REAL NOT NULL DEFAULT 0, created_by TEXT, updated_by TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), archived_at TEXT)");
      await stageSafe("CREATE TABLE IF NOT EXISTS tenant_field_values (id TEXT PRIMARY KEY, definition_id TEXT NOT NULL, company_id TEXT NOT NULL, project_id INTEGER, entity_type TEXT NOT NULL, entity_id TEXT NOT NULL, value_json TEXT NOT NULL DEFAULT 'null', created_by TEXT, updated_by TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), archived_at TEXT, UNIQUE(definition_id,company_id,project_id,entity_type,entity_id))");
      await stageSafe("CREATE INDEX IF NOT EXISTS idx_tenant_field_defs_context ON tenant_field_definitions(company_id,project_id,entity_type,archived_at,rank)");
      ...
      const state = { readyAt: Date.now(), mode:'once-per-isolate' };
      globalThis.__allamoStageReportSchemaState = state;
      return state;
    })().catch((e) => {
      globalThis.__allamoStageReportSchemaPromise = null;
      console.error('[stage-report-schema] falha inesperada', String(e));
      return { readyAt:0, mode:'retry-next-request', error:String(e) };
    });
  }

  await globalThis.__allamoStageReportSchemaPromise;
}
