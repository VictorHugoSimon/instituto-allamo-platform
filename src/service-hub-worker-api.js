...
const shwRequiredTables=['service_hub_systems','service_hub_channels','service_hub_sla_policies','service_hub_tickets','service_hub_ticket_events','service_hub_messages','service_hub_audit_log'];
...
if(path==='service-hub/tickets'&&request.method==='POST'){
  ...
  const b=await shwBody(),projectId=shwClean(b.projectId,120),systemId=shwClean(b.systemId,160)||null,channelId=shwClean(b.channelId,160)||null,source=shwOne(b.source||'manual',shwSource),type=shwOne(b.messageType||'request',shwTypes),priority=shwOne(b.priority||'normal',shwPriorities),title=shwClean(b.title,220);
...
if(path.match(/^service-hub\/tickets\/[^/]+\/status$/)&&request.method==='POST'){
  ...
  const now=new Date().toISOString(),assigned=shwClean(b.assignedTo,180)||null,resolved=(status==='resolved'||status==='closed')?now:(old.resolved_at||null),note=shwRedact(b.note).slice(0,4000);
  await DB.prepare("UPDATE service_hub_tickets SET status=?,assigned_to=COALESCE(?,assigned_to),resolved_at=?,updated_at=? WHERE id=? AND tenant_id=?").bind(status,assigned,resolved,now,id,shwTenant).run();
  await shwEvent(shwTenant,id,'ticket.status_changed',{from:old.status,to:status,note});await shwAudit(shwTenant,'ticket',id,'status_change',{from:old.status,to:status});await logEvent(env,user,'service-hub:chamado-status',id,old.status+' → '+status);
  return json({ok:true,id,status,updated_at:now,resolved_at:resolved});
}
