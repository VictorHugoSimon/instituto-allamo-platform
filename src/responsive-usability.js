...
#arm{width:100vw!important;max-width:100vw!important;overflow-x:hidden!important}
#arm .art{min-height:58px;padding:12px clamp(12px,2.2vw,24px)!important}
#arm .arc{width:100%;max-width:1480px;margin:0 auto;padding:clamp(12px,2.2vw,24px)!important}
#arm .rlist{max-width:100%;overflow-x:auto!important;-webkit-overflow-scrolling:touch}
#arm .rrow{min-width:820px}
#arm .road{min-width:980px}
#arm .modal{padding:16px!important;overflow:hidden!important}
#arm .box{width:min(1100px,calc(100vw - 32px))!important;max-width:1100px!important;max-height:calc(100dvh - 32px)!important;overflow:auto!important;overscroll-behavior:contain}
#arm .actions{align-items:center}
#arm .tabs{overflow-x:auto;white-space:nowrap;padding-bottom:3px;-webkit-overflow-scrolling:touch}
#arm .tabs .arb{flex:0 0 auto}
...
@media(max-width:1023px){
  #arm .arf{grid-template-columns:1fr 1fr!important}
  #arm .grid{grid-template-columns:1fr!important}
...
@media(max-width:767px){
  ...
  #arm .art{position:sticky!important;top:0!important;gap:7px!important}
  #arm .art>b{width:100%;font-size:15px}
  #arm .art>span:not(.sp){display:none}
  #arm .art .sp{display:none}
  #arm .art .arb{flex:1 1 auto;min-height:40px}
  #arm .arf{grid-template-columns:1fr!important}
  #arm .rlist{border:0!important;background:transparent!important;overflow:visible!important}
  #arm .rrow{min-width:0!important;display:grid!important;grid-template-columns:1fr 1fr!important;margin:0 0 10px!important;padding:12px!important;border:1px solid #e4e7ec!important;border-radius:12px!important;background:#fff!important;gap:7px!important}
  #arm .rrow.rhead{display:none!important}
  #arm .rrow>div:first-child{grid-column:1/-1;font-size:14px}
  #arm .road{min-width:760px}
  #arm .modal{padding:0!important;align-items:stretch!important}
  #arm .box{width:100vw!important;max-width:100vw!important;height:100dvh!important;max-height:100dvh!important;border-radius:0!important;padding:16px!important}
  #arm .actions .arb{flex:1 1 130px;min-height:40px}
  ...
  button,.arb,.ard-btn{min-height:40px}
}
@media(max-width:420px){
  #arm .rrow{grid-template-columns:1fr!important}
  #arm .rrow>*{grid-column:1!important}
...
`;
  ...
  function ensureStyle(){
    let s=document.getElementById(STYLE_ID);if(!s){s=document.createElement('style');s.id=STYLE_ID;(document.head||document.documentElement).appendChild(s)}
    if(s.textContent!==css)s.textContent=css;
  }
  function nearestFixed(el){let x=el;for(let i=0;i<10&&x;i++,x=x.parentElement){try{const p=getComputedStyle(x).position;if(p==='fixed')return x}catch(_){}}return null}
  function enhanceReportEditor(){
    const h=[...document.querySelectorAll('h1,h2,h3')].find(x=>x.textContent.trim()==='Editar Status Report');if(!h)return;
    const form=h.closest('form')||h.parentElement;if(!form)return;form.classList.add('allamo-report-editor');
    const fixed=nearestFixed(form);if(fixed){fixed.classList.add('allamo-responsive-modal');let box=form;while(box.parentElement&&box.parentElement!==fixed)box=box.parentElement;box.classList.add('allamo-responsive-modal-box')}
    [...form.querySelectorAll('div')].forEach(d=>{try{const cs=getComputedStyle(d);if(cs.display==='grid'&&d.querySelectorAll('input,select,textarea').length>=2)d.classList.add('allamo-form-grid')}catch(_){}});
    const cancel=[...form.querySelectorAll('button')].find(b=>/^cancelar$/i.test(b.textContent.trim()));const save=[...form.querySelectorAll('button')].find(b=>/^salvar$/i.test(b.textContent.trim()));
    const a=(cancel?.parentElement===save?.parentElement?cancel?.parentElement:(save?.parentElement||cancel?.parentElement));if(a)a.classList.add('allamo-form-actions');
  }
  function wrapWideTables(){
    [...document.querySelectorAll('table')].forEach(t=>{if(t.closest('.allamo-scroll-x'))return;const r=t.getBoundingClientRect();if(r.width>Math.max(360,window.innerWidth-24)){const w=document.createElement('div');w.className='allamo-scroll-x';t.parentNode.insertBefore(w,t);w.appendChild(t)}});
  }
