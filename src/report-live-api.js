...
const lverWrite=['admin','pmo','techlead'].includes(user.role);
const lverId=p=>p+'-'+Date.now().toString(36).toUpperCase()+'-'+crypto.randomUUID().slice(0,5).toUpperCase();
const lverParse=s=>{try{return JSON.parse(s||'{}')}catch(_){return {}}};
const lverScoped=id=>!scope||String(id)===String(scope);
const lverReport=async id=>DB.prepare("SELECT * FROM report_records WHERE id=? AND archived_at IS NULL").bind(id).first();
const lverSnapshot=async(reportId,note)=>{
  const r=await lverReport(reportId);if(!r)return;
  ...
  await DB.prepare('INSERT INTO report_versions(report_id,company_id,project_id,version_no,snapshot_json,change_note,created_by) VALUES(?,?,?,?,?,?,?)')
    .bind(reportId,r.company_id,r.project_id||null,Number(n?.n||1),JSON.stringify({report:{...r,data:lverParse(r.data_json)},roadmap:road}),note||'Atualização',user.name).run();
};

if(path.match(/^report-records\/[^/]+\/duplicate$/)&&request.method==='POST'){
  if(!lverWrite)return json({error:'Sem permissão para criar nova edição'},403);
  const sourceId=decodeURIComponent(path.split('/')[1]),src=await lverReport(sourceId);
  if(!src)return json({error:'Report de origem não encontrado'},404);
  if(!lverScoped(src.company_id))return json({error:'Fora do escopo'},403);
  ...
  const id=lverId('RPT');
  const data=lverParse(src.data_json);data._edition={...(data._edition||{}),previous_report_id:sourceId,created_from:'duplicate',created_at:new Date().toISOString()};
  ...
  await lverSnapshot(id,'Nova edição criada a partir de '+sourceId);
