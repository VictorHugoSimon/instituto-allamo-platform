...
for(const [n,l] of [['allamo-status-report-master-v1','id template mestre'],['__allamoStatusReportMasterSource','fonte literal'],["source:'literal-html-drive'",'origem literal'],['frame.srcdoc=src','HTML literal no iframe'],['applyReport(frame.contentDocument,report)','dados do ciclo aplicados na cópia'],['arm-inline','modo inline'],['min-width:0','container inline sem overflow'],["doc.querySelector('.card.crono')",'cronograma dinâmico usa estrutura original'],['dataset.reportTemplate','versão do template'],['Visão Geral · Evolução do Escopo · Evolução das Horas · Próximos Passos','subtítulo padrão'],['drawCurve(doc,d)','Curva S dinâmica'],['doc.getElementById(\'riscos\')','riscos dinâmicos'],['doc.getElementById(\'proximos\')','próximos passos dinâmicos']])must(viewer,n,l);
...
console.log('OK: HTML mestre recuperável/reparado, cronograma original dinâmico, quatro abas, histórico por ciclos, segregação pública e snapshots encadeados validados.');
