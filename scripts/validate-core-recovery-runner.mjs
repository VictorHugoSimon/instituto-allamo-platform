...
must(portable,"ALLAMO_DEEPEST_D1_RESULTS",'parser desce por envelopes results aninhados');
must(portable,"got[0]?.id!=='dualclima'",'self-test valida linhas reais id/name');
...
if(!String(selfTest.stdout||'').includes('results real com id/name'))throw new Error('Self-test do wrapper portátil não comprovou descida até as linhas reais do D1.');
...
console.log('OK: runner local preserva working tree, usa worktree limpo, gate main/develop via git diff --quiet, backups, gates, parser Wrangler D1 profundo, reparo aditivo, wrapper LF/CRLF/BOM, Stage/Produção e smoke multiempresa.');
