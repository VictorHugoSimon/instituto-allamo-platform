...
const get=async p=>{let last;for(let i=0;i<6;i++){try{const r=await fetch(base+p,{headers:{'cache-control':'no-store'}});const d=await r.json().catch(()=>({}));if(r.ok)return d;last=new Error(`${p}: HTTP ${r.status} ${d.error||''}`)}catch(e){last=e}await sleep(3000)}throw last};
