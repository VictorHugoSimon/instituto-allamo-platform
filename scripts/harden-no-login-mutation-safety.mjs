...
if(!worker.includes(companyRequestMarker)){
  ...
  console.log('OK: onboarding de empresa já exige request_id idempotente.');
}
...
if(!worker.includes(projectRequestMarker)){
  ...
  console.log('OK: onboarding de projeto já exige request_id idempotente.');
}

// Remove a regra antiga que bloqueava o onboarding no modo oficial sem login.
const legacyOnboardingSessionGuard="      if (user.__portal_no_login === true) return json({ error:'Onboarding exige sessão humana autenticada', code:'authenticated_onboarding_required' },403);\n";
if(worker.includes(legacyOnboardingSessionGuard)){
  worker=worker.split(legacyOnboardingSessionGuard).join('');
  changed=true;
  console.log('OK: exigência legada de sessão humana removida do onboarding; identidade PMO sintética permanece auditável.');
}
