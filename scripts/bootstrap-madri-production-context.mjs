...
const CONFIRM='CREATE-MADRI-CONTEXT-PRODUCTION';
...
if(APPLY&&confirmArg!==CONFIRM){
  console.error(`[ABORTADO] Para criar o contexto em Produção use --confirm=${CONFIRM}`);
  process.exit(2);
}
...
function resolveCompany(){
  ...
  if(matches.length>1)throw new Error(`Tenant MADRI ambíguo em Produção (matches=${matches.length}).`);
...
function resolveNucci(companyId){
  ...
  if(matches.length>1)throw new Error(`Projeto NUCCI ambíguo em Produção (matches=${matches.length}).`);
  return {project:matches[0]||null,projects:rows};
}

let company=resolveCompany();
console.log(`[PRODUCTION] Tenant MADRI: ${company?`${company.name} [${company.id}]`:'AUSENTE — será criado como Madri [madri]'}`);
if(!APPLY){
  if(company){const r=resolveNucci(company.id);console.log(`[PRODUCTION] Projeto NUCCI: ${r.project?`${r.project.name} [${r.project.id}]`:'AUSENTE — será criado como Implantação NUCCI ERP'}`)}
  else console.log('[PRODUCTION] Projeto NUCCI: será criado após o tenant MADRI.');
...
if(!company){
  const values={
    ...
    summary:'Implantação NUCCI ERP/TMS — contexto produtivo MADRI',email:'',owner_email:'',grupo:'',billing_to:'',billing_email:'',billing_amount:'',billing_day:'',stakeholders:''
  };
  ...
  if(!company)throw new Error('Falha ao criar/resolver tenant MADRI em Produção.');
  console.log(`[OK] Tenant criado: ${company.name} [${company.id}]`);
}

let {project}=resolveNucci(company.id);
if(!project){
  const values={
    name:'Implantação NUCCI ERP',company_id:company.id,status:'Em andamento',badge:'started',urgency:'Alta',summary:'Implantação do ERP/TMS NUCCI na MADRI',lead:'PMO',start_date:'2026-08-24',meta_date:'',pmo_read:'Atenção',note:'Contexto produtivo autorizado em 2026-09-09',linear_url:''
  };
  ...
  if(!project)throw new Error('Falha ao criar/resolver projeto Implantação NUCCI ERP em Produção.');
  console.log(`[OK] Projeto criado: ${project.name} [${project.id}]`);
}

// O baseline do Plano Mestre é versionado no repositório e usa INSERT OR IGNORE.
...
if(Number(row.n)!==18)throw new Error(`Baseline do Plano Mestre incompleto em Produção: ${Number(row.n||0)}/18.`);
console.log(`[OK] Contexto MADRI produtivo: ${company.name} / ${project.name}; Plano Mestre baseline=18/18.`);
