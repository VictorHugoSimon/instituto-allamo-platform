...
if (APPLY && confirmArg !== CONFIRM) {
  console.error(`[ABORTADO] Para aplicar em Produção use --confirm=${CONFIRM}`);
  process.exit(2);
}
for (const file of [CONFIG, REQ_FILE, TEST_FILE]) {
  if (!fs.existsSync(file)) throw new Error(`Arquivo obrigatório ausente: ${file}`);
}

const decodeJson = file => JSON.parse(zlib.gunzipSync(Buffer.from(fs.readFileSync(file, 'utf8').trim(), 'base64')).toString('utf8'));
...
if (requirements.length !== 86) throw new Error(`Baseline de requisitos inválida: ${requirements.length}/86.`);
if (tests.length !== 54) throw new Error(`Baseline de testes inválida: ${tests.length}/54.`);
if (new Set(requirements.map(x => x.display_id)).size !== 86) throw new Error('IDs duplicados no baseline de requisitos.');
if (new Set(tests.map(x => x.display_id)).size !== 54) throw new Error('IDs duplicados no baseline de testes.');
...
for (const t of tests) {
  if (!['SIT','UAT','E2E'].includes(String(t.test_type || ''))) throw new Error(`Tipo de teste inválido em ${t.display_id}`);
  if (!['P1','P2','P3'].includes(String(t.priority || ''))) throw new Error(`Prioridade inválida em ${t.display_id}`);
}
...
// Fail-closed: este script NUNCA cria empresa ou projeto. O contexto produtivo deve existir previamente.
...
if (matches.length !== 1) throw new Error(`Tenant MADRI não resolvido de forma única em Produção (matches=${matches.length}). Nenhum dado foi alterado.`);
...
const project = nucci.length === 1 ? nucci[0] : (projects.length === 1 ? projects[0] : null);
if (!project) throw new Error(`Projeto MADRI/NUCCI não resolvido de forma única em Produção (projects=${projects.length}, nucci=${nucci.length}). Nenhum dado foi alterado.`);
...
function reqSql(r) {
  ...
  const vals = [id,r.display_id,company.id,project.id,r.origin,r.area,r.subarea,r.requirement,r.priority,r.criticality,!!r.eliminatory,r.source_document,r.target_document,r.section,r.coverage_status,r.gap,r.classification,r.owner,r.action_id,r.test_id,r.evidence,r.acceptance,'baseline-seed-production','baseline-seed-production'];
...
function phaseSql(name,index) {
  ...
  const vals=[id,display,company.id,project.id,index+1,name,'PENDENTE DE VALIDAÇÃO','A confirmar','A confirmar','Sem evidência suficiente','baseline-seed-production','baseline-seed-production'];
...
function readinessSql(category,index) {
  const display=`RDY-${String(index+1).padStart(3,'0')}`, id=`MADRI-${display}`;
  ...
  const vals=[id,display,company.id,project.id,category,`Validar prontidão: ${category}`,'PENDENTE DE VALIDAÇÃO','Pendente','Sem evidência suficiente',true,'baseline-seed-production','baseline-seed-production'];
...
const count=(table,ids)=>Number(query(`SELECT COUNT(*) n FROM ${table} WHERE company_id=${lit(company.id)} AND project_id=${lit(project.id)} AND display_id IN (${ids.map(lit).join(',')});`)[0]?.n||0);
const before={
  requirements:count('madri_requirements',requirements.map(x=>x.display_id)),
  tests:count('madri_tests',tests.map(x=>x.display_id)),
  phases:count('madri_implementation_phases',phaseIds),
  readiness:count('madri_readiness',readyIds),
};
console.log(`[PRODUCTION] Contexto: ${company.name} / ${project.name}`);
console.log(`[PRODUCTION] Baseline esperado: requisitos=86, testes=54, fases=15, readiness=17`);
console.log(`[PRODUCTION] Já presentes: requisitos=${before.requirements}, testes=${before.tests}, fases=${before.phases}, readiness=${before.readiness}`);
if (!APPLY) {
...
if (/\b(?:DELETE|DROP|TRUNCATE)\b/i.test(sql)) throw new Error('Operação destrutiva detectada no lote; abortado.');
const tmp=`.tmp-madri-production-baseline-${process.pid}.sql`;
fs.writeFileSync(tmp,sql,'utf8');
try { run([WRANGLER,'d1','execute',DB,'--remote','--config',CONFIG,'--file',tmp],{capture:false}); }
...
if (after.requirements!==86 || after.tests!==54 || after.phases!==15 || after.readiness!==17) {
  throw new Error(`Seed incompleto em Produção: requisitos=${after.requirements}/86, testes=${after.tests}/54, fases=${after.phases}/15, readiness=${after.readiness}/17.`);
}
console.log(`[OK] Baseline MADRI persistido no D1 Produção: requisitos=${after.requirements}, testes=${after.tests}, fases=${after.phases}, readiness=${after.readiness}.`);
