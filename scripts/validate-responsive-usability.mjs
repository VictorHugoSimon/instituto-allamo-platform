...
new Function(responsive);new Function(admin);new Function(raci);new Function(watchdog);new Function(clientReports);new Function(releases);new Function(clientApi);
...
for(const [n,l] of [['REPORTS PUBLICADOS','painel do cliente'],['published-reports','endpoint publicado'],['Nenhum Report publicado ainda','estado vazio'],['REPORT PUBLICADO','visualização somente leitura']])must(clientReports,n,l);
...
for(const [n,l] of [["status='PUBLICADO'",'somente publicados'],["user.role==='usuario'",'proteção cliente'],['published-reports','API publicada']])must(clientApi,n,l);
for(const f of ['src/report-client-api-guard.js','src/report-admin-navigation.js','src/client-published-reports.js','src/release-history-ui.js','src/raci-visual.js','src/responsive-usability.js'])must(build,f,'injeção '+f);
...
for(const marker of ['__allamoResponsiveUsabilityLoaded','__allamoReportAdminNavLoaded','__allamoRaciVisualLoaded','__allamoClientPublishedReportsLoaded','__allamoReleaseHistoryLoaded'])must(index,marker,'artefato final '+marker);
if(!index.includes('Central de Reports'))throw new Error('Artefato final sem central de Reports.');
console.log('OK: Reports publicados no cliente, Central por projeto, Viradas/Versões, responsividade, RACI, feedback e performance validados.');
