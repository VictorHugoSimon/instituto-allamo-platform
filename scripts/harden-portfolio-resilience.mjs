...
// Falha transitória de uma API nunca pode ser convertida em coleção vazia.
// O último estado válido fica renderizado e uma nova sincronização é agendada.
if(!html.includes('const keepOnError=(name,e)=>')){
  const needle='  async loadData() {\\\n';
  const injected="  async loadData() {\\\n    const keepOnError=(name,e)=>{ console.error('[loadData] '+name,e); if(!this.__dataRetryScheduled){ this.__dataRetryScheduled=true; setTimeout(()=>{ this.__dataRetryScheduled=false; try{ this.loadData(); }catch(_){} },1500); } return null; };\\\n";
...
console.log('OK: falha transitória não apaga carteira/projetos; último estado válido é preservado e a sincronização tenta novamente.');
