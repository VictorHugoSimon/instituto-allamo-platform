...
if(!build.includes('harden-portal-runtime.mjs && node scripts/harden-session-stability.mjs && node scripts/harden-login-autofill.mjs && node scripts/harden-public-client-pwa.mjs && node scripts/enforce-live-first-paint.mjs && node scripts/harden-data-freshness.mjs'))throw new Error('Ordem do build pode reintroduzir sessão/login/cache/PWA antigos.');
console.log('OK: cache, sessão estável entre abas, login/autofill, BFCache, Service Worker e contexto multiempresa revalidam sem bloquear a interface.');
