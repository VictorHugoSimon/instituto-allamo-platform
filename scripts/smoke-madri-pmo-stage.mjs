...
const smokeToken=String(process.env.ALLAMO_SMOKE_TOKEN||'').trim();
const fail=m=>{throw new Error('[MADRI PMO SMOKE] '+m)};
if(!/^https:\/\/allamo-pmo-stage\.pages\.dev$/i.test(base))fail('Smoke destrutivo recusado fora do Stage canônico');
if(!smokeToken)fail('ALLAMO_SMOKE_TOKEN ausente; lixeira/restore exigem sessão autenticada efêmera de Stage');

const authHeaders={authorization:'Bearer '+smokeToken};
const req=async(path,opt={})=>{
  const r=await fetch(base+path,{...opt,headers:{'content-type':'application/json',...authHeaders,...(opt.headers||{})},cache:'no-store'});
  const text=await r.text();let d;try{d=JSON.parse(text)}catch{d=text}
  if(!r.ok)fail(`${opt.method||'GET'} ${path} -> ${r.status} ${typeof d==='string'?d:JSON.stringify(d)}`);
  return d;
};
const bestEffortArchive=async(id)=>{
  if(!id)return;
  try{await fetch(base+'/api/madri-pmo/actions/'+encodeURIComponent(id),{method:'DELETE',headers:{'content-type':'application/json',...authHeaders},cache:'no-store'})}catch{}
};
...
try{
  ...
  id=created.id;if(!id)fail('POST não retornou ID');
  ...
  console.log('[9/12] lixeira e restauração com sessão autenticada...');
  ...
  customId=custom.id;if(!customId)fail('Customização não retornou ID');
  let customs=await req('/api/madri-pmo/customizations');if(!customs.some(x=>x.id===customId))fail('Customização criada não aparece na visão específica');
  await req('/api/madri-pmo/actions/'+encodeURIComponent(customId),{method:'PATCH',body:JSON.stringify({next_step:'Customização editada'})});
  customs=await req('/api/madri-pmo/customizations');if(customs.find(x=>x.id===customId)?.next_step!=='Customização editada')fail('Edição de customização não persistiu');
  ...
  await req('/api/madri-pmo/actions/'+encodeURIComponent(id),{method:'DELETE'});
  await req('/api/madri-pmo/actions/'+encodeURIComponent(customId),{method:'DELETE'});
  const afterPub=await req('/api/public-madri-pmo-report');if(afterPub.actions.some(x=>x.id===id||x.id===customId))fail('Report público incluiu itens arquivados');
  ...
  // Se qualquer passo falhar depois da criação, tenta arquivar os próprios artefatos do smoke.
  await bestEffortArchive(id);
  await bestEffortArchive(customId);
}
