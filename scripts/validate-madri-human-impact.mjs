...
for(const token of ['Mapa de Impacto Humano','Risco de resistência / dificuldade de adoção','Impacto da mudança','A medir','Motivo da aderência','Evidência observada','Nível de confiança','/api/madri-platform/human-impact',"['Muito alta','Alta','Média','Baixa','A confirmar']"])if(!page.includes(token))throw new Error(`Página humana incompleta: ${token}`);
...
console.log('[OK] MADRI Impacto Humano: D1, API, histórico, lixeira, Change Management, gráfico executivo, escala de manualidade e horas A medir validados.');
