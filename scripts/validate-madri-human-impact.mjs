...
const need=[
  ...
  'public/madri-impacto-humano-report/index.html',
...
const reportPage=fs.readFileSync(need[4],'utf8');
const nav=fs.readFileSync(need[5],'utf8');
const ensure=fs.readFileSync(need[6],'utf8');
...
if(/localStorage|sessionStorage/.test(page)||/localStorage|sessionStorage/.test(reportPage))throw new Error('Impacto Humano não pode usar storage do navegador como fonte operacional.');
for(const token of ['Mapa de Impacto Humano','Risco de resistência / dificuldade de adoção','Impacto da mudança','A medir','Motivo da aderência','Evidência observada','Nível de confiança','/api/madri-platform/human-impact',"['Muito alta','Alta','Média','Baixa','A confirmar']",'/madri-impacto-humano-report/'])if(!page.includes(token))throw new Error(`Página humana incompleta: ${token}`);
for(const token of ['Report executivo · Change Management','Tiago Brandão','Impacto da mudança × dificuldade de adoção','Por que o status foi atribuído?','Onde o trabalho manual mais tende a diminuir','/api/madri-platform/human-impact'])if(!reportPage.includes(token))throw new Error(`Report humano incompleto: ${token}`);
if(!nav.includes('/madri-impacto-humano/')||!nav.includes('/madri-impacto-humano-report/')||!nav.includes('Gestão da mudança'))throw new Error('Navegação MADRI não expõe mapa + report de Impacto Humano.');
...
console.log('[OK] MADRI Impacto Humano: D1, API, histórico, lixeira, Change Management, gráfico executivo, aderência explicada, report executivo e navegação validados.');
