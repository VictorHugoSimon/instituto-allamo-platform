...
-- somente metadados PMO, entrada de demandas, cadência, papéis, customizações
-- e auditoria de completude. O isolamento é por empresa + projeto.
...
CREATE TABLE IF NOT EXISTS opr_action_sequence (
...
CREATE TABLE IF NOT EXISTS opr_role_assignments (
  ...
  supplier TEXT NOT NULL DEFAULT 'PENDENTE DE VALIDAÇÃO',
...
CREATE TABLE IF NOT EXISTS opr_completeness_audit (
  id TEXT PRIMARY KEY,
  company_id TEXT NOT NULL,
  project_id INTEGER NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_opr_action_display_id ON opr_action_meta(project_id, display_id) WHERE display_id <> '';
...
CREATE INDEX IF NOT EXISTS idx_opr_audit_project ON opr_completeness_audit(company_id, project_id, audit_date DESC);
