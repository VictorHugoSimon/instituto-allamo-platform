...
export async function handleEscalation(req:Request,env:Env){
  ...
  const id='ESC-'+crypto.randomUUID().slice(0,8).toUpperCase(); const reason=String(body.reason??'insufficient_evidence').slice(0,120);
  const diagnostic=JSON.stringify(sanitizeForStorage(body.diagnostic??{})).slice(0,20000);
  ...
  await emit(env,'ai_escalated',{reason,tenant:ctx.tenantId,target_queue:'suporte-n1'});return{status:'escalated',escalationId:id};
}
