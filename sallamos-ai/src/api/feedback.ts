...
export async function handleFeedback(req: Request, env: Env, _path: string) {
  const ctx = await requireSession(req, env); requirePermission(ctx, 'ai:feedback:create');
  ...
  await emit(env, 'ai_feedback_received', { tenant: ctx.tenantId, solved: body.solved, rating, reason: comment });
  if (!body.solved) await emit(env, 'knowledge_gap_detected', { tenant: ctx.tenantId, response: body.responseId });
