...
export async function embedPending(env: Env): Promise<{ done: boolean; embedded: number }> {
  const rows = await env.META.prepare(
    `SELECT c.id, c.text, c.module, c.version, c.path, c.commit_sha,
            d.status, d.source_type, d.owner
  ...
  await env.VEC.upsert(pending.map((r: any, i: number) => ({
    ...
    values: out.data[i],
    metadata: {
      module: r.module ?? '', version: r.version ?? '', path: r.path ?? '', commit: r.commit_sha ?? '',
      status: r.status ?? '', source_type: r.source_type ?? '', owner: r.owner ?? ''
    }
  })));

  const ids = pending.map((r: any) => r.id);
  const placeholders = ids.map(() => '?').join(',');
  await env.META.prepare('UPDATE knowledge_chunk SET embedded = 1 WHERE id IN (' + placeholders + ')').bind(...ids).run();
