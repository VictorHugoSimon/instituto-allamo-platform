...
const hasApiToken=Boolean(String(process.env.CLOUDFLARE_API_TOKEN||'').trim());
const hasLegacyKey=Boolean(String(process.env.CLOUDFLARE_API_KEY||'').trim());
const hasLegacyEmail=Boolean(String(process.env.CLOUDFLARE_EMAIL||'').trim());
if(!hasApiToken&&!(hasLegacyKey&&hasLegacyEmail))errors.push('Cloudflare auth ausente: use CLOUDFLARE_API_TOKEN ou CLOUDFLARE_API_KEY + CLOUDFLARE_EMAIL');
if((hasLegacyKey&&!hasLegacyEmail)||(!hasLegacyKey&&hasLegacyEmail))errors.push('Cloudflare Global API Key exige o par CLOUDFLARE_API_KEY + CLOUDFLARE_EMAIL');
if(env==='production'){
...
console.log(JSON.stringify({preflight:'ready',environment:env,cloudflareAuth:hasApiToken?'api_token':'api_key_email',integrationConfig:env==='production'?'present':'optional',runtimeEvidence:env==='production'?'required':'optional'},null,2));
