...
if(env==='production'){
  required('SALLAMOS_AUTH_VALIDATE_URL'); required('SALLAMOS_API_BASE'); required('SALLAMOS_AI_STAGE_URL');
...
console.log(JSON.stringify({preflight:'ready',environment:env,cloudflareCredentials:'present',integrationConfig:env==='production'?'present':'optional'},null,2));
